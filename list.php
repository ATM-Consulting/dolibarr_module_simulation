...
$hookmanager->initHooks(array('simulationlist'));
...
$sql.= ' INNER JOIN '.MAIN_DB_PREFIX.'propal_extrafields pe ON (p.rowid = pe.fk_object AND pe.is_simulation=1)';
...
if ($result)
{
	...
	print_barre_liste($langs->trans('ListOfSimulations').' '.($socid?'- '.$soc->name:''), $page, $_SERVER["PHP_SELF"],$param,$sortfield,$sortorder,'',$num,$nbtotalofrecords,'title_commercial.png');
